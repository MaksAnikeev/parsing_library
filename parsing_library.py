...
def parse_tululu(start_id, end_id):
    for i in range(start_id, end_id+1):
        ...
        try:
            ...
            parse_book_page(response=title_response, number=i)
...
def download_books():
    ...
    for i in numbers.split(','):
        ...
        book_name = soup.find('table').find('h1').text
        title_name = book_name.split('::')[0].strip()
        download_image(url=full_image_link, filename=title_name, folder='books/')

        text_url = f'https://tululu.org/txt.php?id={i.strip()}'
        text_response = requests.get(text_url)
        download_txt(response=text_response, filename=title_name, folder='books')


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('start_id', type=int, help='номер с которого начать')
    parser.add_argument('end_id', type=int, help='номер которым закончить')
    args = parser.parse_args()

    parse_tululu(start_id=args.start_id, end_id=args.end_id)
